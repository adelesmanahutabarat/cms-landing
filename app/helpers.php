...
if(!function_exists('pic_channels')){
    function pic_channels(){
        $landing_ids = UserPIC::selectRaw('landing_id')->where('user_id', auth()->user()->id)->get()->pluck('landing_id');
        $channels = Parameter::selectRaw('param_key')->whereIn('id', $landing_ids)->get()->pluck('param_key');
...
if (!function_exists('excerpt')) {
     ...
     * Prepare the Column Name for Lables.
     */
    function excerpt($text,$limit=30,$pemisah='...')
    {
        return \Illuminate\Support\Str::limit($text,$limit,$pemisah);
    }
}
